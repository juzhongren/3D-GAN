...
class WGAN_GP(object): 
   ...
   """
    ...
    def calc_gradient_penalty(self, real_data, generated_data):
        ...
        alpha = torch.rand(b_size, 1, 64, 64)
    ...
    def training_epoch(self, dataloaders,max_epochs = 1000,
                       num_epochs_pre=0,pretrainedG=' ', pretrainedD=' ',datasetname=" ",index = 0):
        """
        ...
        if pretrainedG != ' ':
            ...
            pre_epoch = int(re.split("epoch", pretrainedG)[1])
            print("第{}epoch".format(pre_epoch))
        ...
        if pretrainedD != ' ':
            ...
            #打开以前的判别器的loss
            if not os.path.exists(datasetname + "loss_history/loss_history2/loss_history_{}".format(index)):
                os.makedirs(datasetname + "loss_history/loss_history2/loss_history_{}".format(index))
            root_dir = datasetname + "loss_history/loss_history2/loss_history_{}".format(index)
            # recall the loss history to continue
        ...
        for epoch in range(1,max_epochs+1):
            ...
            for fold in range(10):
                ...
                for phase in ['train', 'val']:
                    ...
                    for lr_data, hr_data in dataloaders[phase][fold]:
                        ...
                        lr_patches = self.inputdata(lr_patches)
                        hr_patches = self.inputdata(hr_patches)
                        ...
                        if phase == 'train':
                            ...
                            with torch.set_grad_enabled(True):
                                ...
                                sr_patches = self.outputdata(sr_patches)
                                ##########################
                                # sr_patches = self.netG(lr_patches)
                                # # Train D
                                # self.optimizerD.zero_grad()
                                ...
                                # self.optimizerG.step()
                                # ###########################
                                # # sr_patches, D_loss, G_loss, loss = self.updateDG(lr_patches, hr_patches)
                                # sr_patches = self.outputdata(sr_patches)
                                step += 1
                            # # 每500步打印信息
                            # # This print out is only for early inspection
                            # if (step % 500) == 0:
                            #     print('epoch:{}---Step: {}, loss= {:.4f}, D_loss= {:.4f}, G_loss= {:.4f}'.format(epoch,step, loss.item(),
                            #                                                                           D_loss.item(),
                            #                                                                           G_loss.item()))
                            # statistics
                            # 取loss值
                    ...
                    if phase == 'val':
                        ...
                        print('fold No. {} {} period. Mean main loss: {:.4f}. Mean discriminator loss: {:.4f}.'.format(
            ...
            if (epoch % 10) == 0:
                ...
                if not os.path.exists(datasetname+"models/model2/model_{}".format(index)):
                    os.makedirs(datasetname+"models/model2/model_{}".format(index))
                root_dir = datasetname+"models/model2/model_{}".format(index)
                # save intermediate models for singal GPU and multi GPU
                if self.ngpu > 1:
